...
$(function() {
	...
	//点击search按钮触发查询
	$("#show").click(function() {
		var bugId = $(".RoleId").val();
		var name = $(".userName").val();
		getBugList(bugId, name, 1);
	});
	//上一页
	$(".chevron-left").click(function() {
		if (searchObj.startPage == 1) {
			...
			getBugList(searchObj.bugId, searchObj.name, searchObj.startPage - 1);
		}
	});
	...
	$(".step-backward").click(function() {
		if (searchObj.startPage == 1) {
			...
			getBugList(searchObj.bugId, searchObj.name, 1);
		}
	});
	...
	$(".step-forward").click(function() {
		if (searchObj.startPage == searchObj.pages) {
			...
			getBugList(searchObj.bugId, searchObj.name, searchObj.pages);
		}
	});

	//处理下拉框改变
	$(".NUM .pages").change(function() {
		getBugList(searchObj.bugId, searchObj.name, parseInt($(this).val()));
	});



	function getBugList(bugId, bugname,startPage) {
		searchObj.bugId = bugId;
		searchObj.name = bugname;
		...
		$.ajax({
			...
			data: "bugId=" + bugId + "&name=" + bugname + "&startPage=" + startPage + "&pageSize=10",
